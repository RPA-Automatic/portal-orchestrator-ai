...
const resources = ['Atlas', 'Nova', 'Flux', 'Sentinel'].map((name, index) => ({id: `agent-${index}`, name, kind: 'agent', content: 'Instruções de exemplo para revisão visual do catálogo.', config: {role: ['Planejamento', 'Arquitetura', 'Automação RPA', 'Qualidade e segurança'][index]}, created_at: '2026-09-13T12:00:00Z'}));
